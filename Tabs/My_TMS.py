...
def render_page(role):
        ...
        if not df.empty:
            ...
            st.dataframe(
                ...
                column_config={
                    ...
                    "reference": "Ref #",  # Show the reference code clearly
    ...
    # Add reference to the display order
    ...
    if role == "View Only":
        ...
        with st.form("tms_entry_v3", clear_on_submit=True):
            ...
            with c2:
                ...
                # If user selects "Add New", show a text input
                custom_medium = ""
                if selected_medium == "➕ Add New...":
                    custom_medium = st.text_input("Type New Medium Name (e.g. eSewa, Nabil Bank)")
            ...
            # NEW: REFERENCE AND REMARKS
            r1, r2 = st.columns(2)
            t_ref = r1.text_input("Reference Code", placeholder="Bank Txn ID / Cheque #")
            t_remark = r2.text_input("Entry Remarks", placeholder="e.g. Sold for profit")
            ...
            if st.form_submit_button("💾 Commit to Ledger", type="primary", use_container_width=True):
                try:
                    with conn.session as s:
                        ...
                        s.commit()
                    st.success(f"✅ Logged {t_type} via {t_medium} with Ref: {t_ref}")
                    st.rerun()
                except Exception as e:
